...
Inoltre dagli \emph{Screen} è possibile intervenire sulla gara, accelerando o decelerando la velocità di simulazione e forzando un 
concorrente a rientrare ai box.
...
Il \textbf{Competition\_Computer} si appoggia a due risorse per l'archiviazione e l'organizzazione dei dati:
\begin{description}
...
\item{Placement\_Handler}: come dice il nome, questo package è pensato per gestire la classifica.
Al suo interno è presente {\textbf{SOCT\_Array}} che \`{e} la parte pi\`{u} alta di una struttura utilizzata per il supporto alla creazione della classifica.
Nel punto pi\`{u} basso c'\`{e} \textbf{Placement\_Table}, unit\`{a} finalizzata a raccogliere i tempi di lap dei concorrenti. A gestire questa 
tabella virtuale c'\`{e}\\ \textbf{Synch\_Ordered\_Placement\_Table}, che permette di mantenere la tabella ordinata in base ai tempi e offre un set
di metodi per il reperimento dei dati. Infine \textbf{SOCT\_Array} (SOCT sta per Synch Ordered Placement Table) \`{e} un array in cui
...
Quanto descritto costituisce le fondamenta di \textbf{Competition\_Computer}. Il metodi offerti navigano queste strutture per ottenere
i dati richiesti:
\begin{description}
...
\item{\textbf{procedure Get\_Lap\_Placement(Lap : INTEGER;\\ TimeInstant : FLOAT;\\ CompetitorID\_InClassific : out INTEGER\_ARRAY\_POINT;\\ 
...
\end{description}
\end{itemize}
Il \textbf{Competition\_Monitor}, (come vedremo poi pi\`{u} in dettaglio \`{e} una sotto-componente di \emph{Monitor}) fa riferimento al \textbf{Competition\_Computer} per 
...
\subsection{Analisi della concorrenza}
	%. analisi dell'interazione risorse e task (senza menzionare la distribuzione)
	  %Competitor\_Task - Synch_Checkpoint
