...
in cui le ultime \emph{N} auto (con \emph{N} pari alla molteplicità del \textbf{Path}) sono previste lasciare il tratto;
...
Quindi ogni concorrente inizi ad percorrere una traiettoria, dovrà salvare in una posizione
...
che il suo tempo di arrivo sia maggiore almeno ad uno dei tempi di liberazione segnati
nella lista. Solo in questo caso potrà essere certo che la traiettoria può ancora ospitare auto.
In tutte le traiettorie di un tratto siano sovraffollate, il concorrente dovrà
...
evitare le possibili situazioni di stallo e di sorpasso impossibile. Infatti per attraversare un tratto di
...
più un concorrente e che quel concorrente possieda solo l'accesso alla risorsa
interessata, impedendo l'accumulo di risorse (una delle cause di stallo viste
nel paragrafo \ref{stalli}).
...
\begin{description}
\item{Scelta traiettoria (\textbf{Path}):}\\
...
\item Confronta l'istante di uscita dato tale tempo di attraversamento con l'istante di liberazione
segnato sul tratto;
\item Se l'istante di liberazione è inferiore (il che significa che il tratto sarà già libero quando l'auto proverà ad attraversarlo),
...
la lista di istanti di liberazione. Se tutti gli istanti segnati sono maggiori dell'istante di ARRIVO del concorrente, significa che 
...
\emph{l'istante di liberazione del tratto}) + \emph{il tempo che rappresenta la distanza minima fra due auto per evitare tamponamenti} (costante).
\end{itemize}
...
di arrivo di quello del concorrente;
\end{itemize}
\item Altrimenti l'utente deve uscire dal circuito per evitare collisioni e venire squalificato.
\end{itemize}
\item{Attraversamento tratto:}\\
...
anomali. \`{E} necessario, tuttavita, dimostrare che l'algoritmo sia corretto. Per farlo
\begin{itemize}
...
\item mentre \emph{x} sta valutando la traiettoria da percorrere e prima che abbia potuto segnare l'istante di arrivo
sulle code di tutti i checkpoint, si ha che $t_y$ < $t_x$ e \emph{y} inizia quindi a valutare;
\end{itemize}
\textbf{Condizione 2}:\\
\begin{itemize}
...
Non si presentano anomalie poichè \emph{x} ha analizzato i \textbf{Path}, aggiornato il tempo di uscita su quello scelto e aggiornato
gli istanti di arrivo su tutti i \textbf{Checkpoint}.
\end{proof}
...
Assumiamo un circuito a \emph{S} checkpoint come istante di partenza per un concorrente generico \emph{y} $t_n$ al tratto \emph{n}. Il \textbf{Checkpoint} \emph{n} avrà quindi nella coda associata
alla posizione dedicata a \emph{y} l'istante di tempo $t_n$. I checkpoint successivi avranno nella coda associata alla posizione per \emph{y}
l'istante $t_n+\epsilon$ con $\epsilon$ monotonicamente cresente di checkpoint in checkpoint.
...
checkpoint \emph{n}.}\\
	Se nella coda \emph{n} ci sono \emph{x} in testa seguito da \emph{y}, avremo che
$t_{x,n}<t_{y,n}$, poich\`{e} l'ordine della coda \`{e} 
	determinato dai tempi di arrivo segnati.
	Se \emph{x} \`{e} in attesa sulla coda \emph{n}, vuol dire che all'iterzione precedente avr\`{a} aggiornato per
ultimo l'istante segnato sulla coda del \textbf{Checkpoint} \emph{n-1} e di conseguenza $t_{x,n-1}>t_{x,n}$ (come visto precedentemente).
	\emph{y} invece \`{e} in attesa effettiva sulla coda \emph{n-1}, quindi avr\`{a}
aggiornato la coda del tratto successivo con un valore dato da $t_{y,n} + \delta_t$, 
	con $ \delta_t$ un valore minimo diverso da zero previsto per
l'attraversamento da \emph{n-1} a \emph{n}. Quindi $t_{y,n-1}<t_{y,n}$.
	L'ipotesi di stallo prevede che nella coda \emph{n} vi sia primo il concorrente \emph{y} e poi \emph{x},
quindi $t_{y,n}<t_{x,n}$.
	Avremo quindi:\\
	$t_{x,n}<t_{x,n-1}<t_{y,n-1}<t_{y,n}<t_{x,n}$\\\\
...
il proprio tempo (tempo in cui libererà il tratto). 
...
opportunamente i tempi di attraversamento.
