...
--   RaceTrackTemp : Circuit.RACETRACK_POINT :=Get_Racetrack("./obj/racetrack.xml");
   ...
   compStats2 : COMP_STATS_POINT := new COMP_STATS;
   compStats3 : COMP_STATS_POINT := new COMP_STATS;
...
--creazione giri migliori e tempi su settore migliori per scopi di test
   ...
   Common.Set_Checkpoint(compStats3, 0);
   Common.Set_Sector(compStats3, 1); -- TODO, non abbiamo definito i sector, ritorna sempre uno.
   Common.Set_Lap(compStats3, 0);
   Common.Set_Tyre(compStats3, 0.99);
   Common.Set_LengthPath(compStats3, 666.0);
   Common.Set_Time(compStats3, 654.0);
   Common.Set_LastCheckInSect(compStats3, true);
   Common.Set_Gas(compStats3,1984.0);
   obc.Add_Data(compStats3);




   Common.Set_Checkpoint(compStats2, 1);
   -- Ada.Text_IO.Put_Line("POST setcheckpoint PRE setSector");
   Common.Set_Sector(compStats2, 2); -- TODO, non abbiamo definito i sector, ritorna sempre uno.
   ...
   Common.Set_Lap(compStats2, 0);
   Common.Set_Tyre(compStats2, 0.15);
   Common.Set_LengthPath(compStats2, 21.0);
   -- Ada.Text_IO.Put_Line("POST settyre PREset time");
   Common.Set_Time(compStats2, 210.0);
   Common.Set_LastCheckInSect(compStats2, false);
   Common.Set_Gas(compStats2,1.0);
   obc.Add_Data(compStats2);
   --altro settore
   Common.Set_Checkpoint(compStats, 2);
   -- Ada.Text_IO.Put_Line("POST setcheckpoint PRE setSector");
   Common.Set_Sector(compStats, 2); -- TODO, non abbiamo definito i sector, ritorna sempre uno.
   --Common.Set_Lap(42); -- TODO, non ho ancora un modo per sapere il numero di giro
                                                   --commentato- da correggere il ripo di gaslevel ONBOARDCOMPUTER.Set_Gas(compStats, carDriver.auto.GasolineLevel);
                                    -- Ada.Text_IO.Put_Line("POST setsector PRE set tyre");
   Common.Set_Lap(compStats, 0);
   Common.Set_Tyre(compStats, 0.15);
   Common.Set_LengthPath(compStats, 42.0);
   -- Ada.Text_IO.Put_Line("POST settyre PREset time");
   Common.Set_Time(compStats, predictedTime);
   Common.Set_LastCheckInSect(compStats, true);
